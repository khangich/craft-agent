...
- critic_agent: Handle patch from github_agent for quality checking. It will only run after the github_agent has given a patch.
...
GITHUB_AGENT_PROMPT = """You are a GitHub agent. Your task is to review the comments and diffs on a specified pull request (PR) and generate a GitHub patch based on the feedback provided. Follow these steps meticulously:
...
CRITIC_AGENT_PROMPT = """You are a Critic Agent. Your task is to review the patch generated by the github_agent based on the given task. Follow these steps meticulously:

1. Review the Generated Patch: Carefully examine the patch file generated by the gitHub_agent.
2. Ensure that the patch addresses all the feedback provided in the comments and reflects the necessary changes in the diffs.
4. Provide Justification: Justify the correctness and quality of the patch. Highlight how well the patch addresses the feedback and any improvements made.

Output Format:
- Provide the justification for looking good case as "LGTM. TERMINATE".
- Provide the justification for not looking good case as "DOESN'T LGTM. TERMINATE".
- Do not include any additional information other than the justification status.
"""
...
group_chat = autogen.GroupChat(
    agents=[user_proxy, github_agent, critic_agent],
    messages=[],
